...
trait FirestoreClientDocRefFAttempt[F[_]] {
  ...
  def set[A](doc: A, docRef: DocumentReference)(implicit
    ...
    attemptBlocking(F.delay(docRef.set(encoder.encode(doc).document).get()).|>(F.as(_, doc)))

  def get[A](docRef: DocumentReference)(implicit
      F: Sync[F],
      decoder: FirestoreDecoder[A]
  ): F[Either[FirestoreError, A]] =
    attemptBlocking(F.delay(docRef.get().get()).|>(F.map(_)(ds => decoder.decode(codec.FirestoreDocument(ds.getData)))))
