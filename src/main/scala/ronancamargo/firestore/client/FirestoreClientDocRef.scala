...
case class FirestoreClientDocRef(private val firestore: Firestore) {
  ...
  def del[F[_]](docRef: DocumentReference)(implicit F: Async[F]): F[Either[FirestoreError, Unit]] =
  ...
  def attemptBlock[F[_], A](fa: F[A])(implicit F: Async[F]): F[Either[FirestoreError, A]] =
